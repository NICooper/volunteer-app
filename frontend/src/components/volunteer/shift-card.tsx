...
export default function ShiftCard({ shift, onPress }: { shift: ShiftOverview, onPress: () => void }) {
  const volunteersNeeded = shift.numOpenings - shift.numApproved;
  const duration = intervalToDuration({ start: shift.startTime, end: shift.endTime });
  const durationString = formatDuration(duration, { format: ['hours', 'minutes'], zero: false });

  return (
    <Card mode='outlined' style={styles.card} onPress={onPress}>
      <Card.Title
        title={shift.name}
        subtitle={shift.orgName}
        titleVariant='titleMedium'
        subtitleVariant='bodyMedium'
        left={() => <Avatar.Text label={shift.name.charAt(0).toLocaleUpperCase()} size={48} />}
        right={() => <Text variant='bodySmall'>{'Needs: ' + volunteersNeeded + '\n' + durationString}</Text>}
      />
